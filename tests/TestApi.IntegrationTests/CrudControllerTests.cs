...
public sealed class CrudControllerTests : IClassFixture<TestApiFactory>
{
    ...
    [Fact]
    public async Task List_returns_database_products_in_stable_order()
    {
    ...
    [Fact]
    public async Task Create_persists_product_and_returns_created_location()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
    ...
    [Fact]
    public async Task Update_changes_the_persisted_product()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        await _factory.Database()
            .Recreate()
    ...
    [Fact]
    public async Task Update_and_delete_return_not_found_for_missing_products()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        await _factory.RecreateDatabaseAsync(cancellationToken);
        using var client = CreateAuthenticatedClient();
        ...
        using var updateResponse = await client.PutAsJsonAsync(
            ...
            cancellationToken);
        using var deleteResponse = await client.DeleteAsync("/api/products/999", cancellationToken);
    ...
    [Fact]
    public async Task Invalid_create_request_returns_bad_request_without_saving()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        await _factory.RecreateDatabaseAsync(cancellationToken);
        using var client = CreateAuthenticatedClient();
